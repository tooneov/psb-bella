// api/bella.js — robust Node function for Bella (no Edge runtime)
// Uses the exact mapping Relevance support gave you:
...
function authVariants() {
  // Try the common Authorization header formats Relevance accepts
  const base = [
...
function extractReply(p){
  if (!p) return null;
  if (Array.isArray(p.steps)) for (let i=p.steps.length-1; i>=0; i--){ const s=p.steps[i]; const cand=s?.assistant_reply||s?.output||s?.message||s?.text; const str=typeof cand==="string"?cand:(cand&&cand.text); if (str && String(str).trim()) return String(str).trim(); }
  if (Array.isArray(p.messages)) for (let i=p.messages.length-1; i>=0; i--){ const m=p.messages[i]; const role=(m.role||m.sender||"").toLowerCase(); if (role.includes("assistant")){ const cand=m.text||m.message||m.content||m.output; if (cand && String(cand).trim()) return String(cand).trim(); } }
  const flat=p.assistant_reply||p.reply||p.output||p.message||p.text; if (flat && String(flat).trim()) return String(flat).trim();
  return null;
}
...
async function triggerAgent(body){
  ...
  for (const host of HOSTS){
    for (const h of authVariants()){
      const r = await tryFetch(`${host}/latest/agents/tasks/trigger`, { method:"POST", headers:h, body:JSON.stringify(body) });
  ...
  for (const host of HOSTS){
    for (const h of authVariants()){
      const r = await tryFetch(`${host}/latest/agents/trigger`, { method:"POST", headers:h, body:JSON.stringify(body) });
